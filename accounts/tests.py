...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        user = User.objects.create_user(
            username='testuser2', email='test@test.com', password='testpass2')
...
class TestLoginPage(BaseTestCase):
    ...
    def test_login_page_login_wrong_user_or_pass(self):
        response = self.client.post(
            '/accounts/login', {'username': 'testuser2', 'password': 'wrongpass'})
        self.assertContains(response, 'Username or password is incorrect.')

    def test_login_page_login(self):
        response = self.client.post(
            '/accounts/login', {'username': 'testuser2', 'password': 'testpass2'}, follow=True)
